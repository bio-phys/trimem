...
struct CellList : NeighbourListT<exclusion>
{
  //! cells -> points lookup
  ...
  std::array<real, 3> r_list;
  //! box
  std::vector<std::array<real, 3>> box;
  //! cell pairs
  std::vector<std::pair<int, int>> cell_pairs;

  CellList(const TriMesh& mesh, real rlist, real box_eps=1.0e-6)
  {
      // compute box-dims
      std::array<real, 3> box_min = { std::numeric_limits<real>::max() };
      std::array<real, 3> box_max = { -1 };
  ...
  distance_matrix(const TriMesh& mesh, const real& rlist) const override
  {
